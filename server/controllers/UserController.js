...
const signin = async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign({ username: existingUser.email},process.env.SECRET_KEY);
...
const allUserJson = async (req, res) => {
  ...
  const changeUserStatus = async (req, res) => {
    try {
        const updatedUser = await users.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true });
        if (!updatedUser) {
...
const changeUserRole = async (req, res) => {
    try {
        const updatedUser = await users.findByIdAndUpdate(req.params.id, { status: req.body.role }, { new: true });
        if (!updatedUser) {
            return res.status(404).send('User not found');
        }
        res.json(updatedUser);
