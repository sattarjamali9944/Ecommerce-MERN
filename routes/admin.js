...
const {
	...
	getSubAttributes,
} = require("../server/controllers/AttributeController");
const {
	addContactPost,
	allContactJson,
	contacts,
...
const {
	addShippingBilling,
	postShippingBilling,
} = require("../server/controllers/shippingBillingController");
...
const uploadCategoryImage = multer({
	storage: createMulterStorage("categories"),
});
...
router.post(
	"/add-user",
	registerValidationRule.RegisterValidation,
...
router.post(
	...
	userController.uploadUserImageFunction
);
...
router.post(
	"/createProduct",
	uploadProductImage.single("files"),
	[
		body("title")
			.isLength({ min: 3 })
...
router.get(
	...
	cors(),
	productController.ProductCategoryWise
);
...
router.post(
	"/createbanner",
	uploadBannerImage.single("files"),
	[
		body("title")
			.isLength({ min: 3 })
			.withMessage("Title must be at least 3 characters long"),
